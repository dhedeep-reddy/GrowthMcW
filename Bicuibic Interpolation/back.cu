...
__device__ __forceinline__ float cubic_gpu(float x)
{
    ...
    float t1 = (a + 2.f)*x*x*x - (a + 3.f)*x*x + 1.f;   
    float t2 =  a*x*x*x - 5.f*a*x*x + 8.f*a*x - 4.f*a;  
    ...
    float scaleX, float scaleY)
{
    ...
    float Wx[4], Wy[4];
    #pragma unroll 4
    for (int i = -1; i <= 2; i++) Wx[i+1] = cubic_gpu(i - dx);
    #pragma unroll 4
    for (int i = -1; i <= 2; i++) Wy[i+1] = cubic_gpu(dy - i);
    ...
    for (int m = -1; m <= 2; m++) {
        ...
        for (int n = -1; n <= 2; n++) {
            ...
            if (sy >= 0 && sy < SMEM_H && sx >= 0 && sx < SMEM_W)
                ...
                int gx = min(max(ix + n, 0), inW - 1);
                int gy = min(max(iy + m, 0), inH - 1);
                val = input[gy * inW + gx];
            }
            sum += val * Wx[n+1] * Wy[m+1];
        }
    }
    ...
    float scaleX, float scaleY)
{
    ...
    for (int i = tid; i < total; i += TILE * TILE)
    {
        int sy = i / smem_w;
        int sx = i % smem_w;
        float val = smem[sy][sx];
        if (val != 0.f)
        {
            int gx = min(max(ix0 + sx, 0), inW - 1);
            int gy = min(max(iy0 + sy, 0), inH - 1);
            atomicAdd(&grad_input[gy * inW + gx], val);
        }
    }
}
