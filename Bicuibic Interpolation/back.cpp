...
Mat bicubic_backward(const Mat& input, const Mat& grad_ones_vec, int outW, int outH)
{
    int inW = input.cols;
    int inH = input.rows;
    ...
    for (int y = 0; y < outH; y++)
    {
        for (int x = 0; x < outW; x++)
        {
            ...
            float g = grad_ones_vec.at<float>(y, x);
...
int main()
{
    ...
    // Forward 
    Mat opencv_out = bicubic_opencv(gray, outW, outH);
    Mat manual_out = bicubic_forward(gray, outW, outH);

    validate(opencv_out, manual_out, "Forward: Manual vs OpenCV");

    // Backward
    Mat grad_one = Mat::ones(outH, outW, CV_32F);
    Mat grad_in  = bicubic_backward(gray, grad_one, outW, outH);
