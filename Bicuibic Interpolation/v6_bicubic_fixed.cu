    ...
    float scaleX, float scaleY)
{
    ...
    // per-thread state (zero for out-of-bounds threads)
    ...
    if (out_x < outW && out_y < outH)
    {
        ...
        lx = ix - ix0;
        ly = iy - iy0;
        g  = __ldg(&grad_output[out_y * outW + out_x]);
    }
    ...
    for (int m = 0; m < 4; m++) {
        ...
        for (int n = 0; n < 4; n++) {
            ...
            for (int stride = WARP_SIZE / 2; stride >= 1; stride >>= 1)
            {
                ...
                float  peer_contrib = __shfl_down_sync(FULL_MASK, contrib, stride);
                int    peer_sy      = __shfl_down_sync(FULL_MASK, sy,      stride);
                int    peer_sx      = __shfl_down_sync(FULL_MASK, sx,      stride);

                if (peer_sy == sy && peer_sx == sx)
                    contrib += peer_contrib;
            ...
            if (lane == 0) {
                ...
                int prev_sy = __shfl_up_sync(FULL_MASK, sy, 1);
                int prev_sx = __shfl_up_sync(FULL_MASK, sx, 1);

                is_owner = !(prev_sy == sy && prev_sx == sx);
            }

            if (is_owner && contrib != 0.f) {
                if (sy >= 0 && sy < smem_h && sx >= 0 && sx < smem_w)
                    atomicAdd(&smem[sy][sx], contrib);
                else {
                    // out-of-smem-bounds: go direct to global
                    int gx = min(max(ix + n - 1, 0), inW - 1);
                    int gy = min(max(iy + m - 1, 0), inH - 1);
...
Mat bicubic_forward_cuda(const Mat& input, int outW, int outH)
{
    ...
    cout << "[Forward ] kernel: " << ms << " ms  |  " << (flops / 1e9) / (ms / 1000.f) << " GFLOPS" << endl;
