...
// Forward kernel: smem + unroll + 2x2 register tiling
    ...
    float scaleX, float scaleY)
{
    ...
    // 2x2 register tile: each thread owns 2 cols and 2 rows
    ...
    // write all 4 results to global in one pass
    #pragma unroll 2
    for (int rr = 0; rr < 2; rr++)
    #pragma unroll 2
    for (int rc = 0; rc < 2; rc++)
...
Mat bicubic_forward_cuda(const Mat& input, int outW, int outH)
{
    ...
    // grid halved: each thread covers 2x2 outputs
...
Mat bicubic_backward_cuda(const Mat& grad_out_mat, int inW, int inH)
{
    ...
    bicubic_backward_kernel<<<grid, block>>>(
        d_grad_output, outW, outH, d_grad_input, inW, inH, scaleX, scaleY);
