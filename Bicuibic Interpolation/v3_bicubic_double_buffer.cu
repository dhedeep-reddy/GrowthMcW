...
#define TILE      16
#define MAX_SCALE 2
#define SMEM_W    (TILE * MAX_SCALE + 4)
#define SMEM_H    (TILE * MAX_SCALE + 4)
...
// Forward kernel: smem + unroll + double-buffer
...
{
    // Two buffers for ping-pong prefetch
    ...
    int smem_w = min((int)ceilf(TILE * scaleX) + 4, SMEM_W);
    int smem_h = min((int)ceilf(TILE * scaleY) + 4, SMEM_H);
    ...
    int next_ix0 = ix0 + (int)ceilf(TILE * scaleX);
    ...
    for (int i = tid; i < total; i += TILE * TILE)
    {
        ...
        int gy = min(max(iy0 + sy,      0), inH - 1);
        nxtBuf[sy][sx] = input[gy * inW + gx];
    }

    // Pointer swap 
    float (*tmp)[SMEM_WP] = curBuf;
    curBuf = nxtBuf;
    nxtBuf = tmp;
}


// Backward kernel: smem + unroll
__global__ void bicubic_backward_kernel(
    ...
    float scaleX, float scaleY)
{
    __shared__ float smem[SMEM_H][SMEM_WP];
