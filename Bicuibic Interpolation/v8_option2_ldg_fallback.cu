    ...
    float scaleX, float scaleY)
{
    ...
    int ox_start = max(0,       (int)floorf((in_x - 1.5f) * inv_scaleX - 0.5f));
    int ox_end   = min(outW-1,  (int)floorf((in_x + 2.5f) * inv_scaleX - 0.5f) + 1);
    int oy_start = max(0,       (int)floorf((in_y - 1.5f) * inv_scaleY - 0.5f));
    ...
    for (int oy = oy_start; oy <= oy_end; oy++)
    {
        ...
        for (int ox = ox_start; ox <= ox_end; ox++)
        {
            ...
            float g = __ldg(&grad_output[oy * outW + ox]);
            grad_sum = __fmaf_rn(g, wx * wy, grad_sum);
        }
    }
...
Mat bicubic_backward_cuda(const Mat& grad_out_mat, int inW, int inH)
{
    ...
    // Grid covers input space — one thread per input pixel.
    dim3 block(TILE, TILE);
    ...
    // No smem needed — pass 0 bytes.
    bicubic_backward_kernel<<<grid, block, 0>>>(
...
int main()
{
    ...
    // save_bin(cuda_fwd, "opt2_forward.bin");
    // save_bin(cuda_bwd, "opt2_backward.bin");
