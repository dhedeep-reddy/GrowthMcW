...
// Backward kernel: warp-level reduction via __shfl_xor_sync 
    ...
    float scaleX, float scaleY)
{
    ...
    for (int m = 0; m < 4; m++) {
        ...
        for (int n = 0; n < 4; n++) {
            ...
            float nb_contrib = __shfl_xor_sync(FULL_MASK, contrib, 1);
            int   nb_sy      = __shfl_xor_sync(FULL_MASK, sy,      1);
            int   nb_sx      = __shfl_xor_sync(FULL_MASK, sx,      1);
            bool  same_cell  = (nb_sy == sy && nb_sx == sx);
            bool  is_lower   = !(lane & 1);

            if (same_cell && is_lower)  contrib += nb_contrib;  
            if (same_cell && !is_lower) contrib  = 0.f;         
